 ...
 * src/main/java/org/opentripplanner/common/geometry/SphericalDistanceLibrary.java
 */
public abstract class SphericalDistanceLibrary {
	...
	public static final double fastDistance(Coordinate point, LineString lineString) {
		...
		Point point2 = JTSFactoryFinder.getGeometryFactory().createPoint(new Coordinate(lon, lat));
	...
	private static LineString equirectangularProject(LineString lineString, double cosLat) {
		...
		return JTSFactoryFinder.getGeometryFactory().createLineString(coords2);
	}
	 ...
	 *            Distance in meters.
	 * @return The number of degree for the given distance. For degrees
	 *         latitude, this is nearly correct. For degrees longitude, this is
	 *         an overestimate because meridians converge toward the poles.
	 */
	...
	/**
	 * @return the approximate number of meters for the given number of degrees
	 *         latitude. If degrees longitude are supplied, this is an
	 *         overestimate anywhere off the equator because meridians converge
	 *         toward the poles.
	 */
